...
func (e *WorkItemListExpression) First() *work_item.WorkItemExpression {
	if e.err != nil { panic(e.err) }
	if e.value == nil || len(e.value.Items()) == 0 { return work_item.NewWorkItemExpression(nil) }
...
func (e *WorkItemListExpression) Get(index int) *work_item.WorkItemExpression {
	if e.err != nil { panic(e.err) }
	if e.value == nil || index < 0 || index >= len(e.value.Items()) { return work_item.NewWorkItemExpression(nil) }
	return work_item.NewWorkItemExpression(e.value.Items()[index])
}

func (e *PlatformExpression) WorkItemList() *WorkItemListExpression {
	if e.err != nil { return &WorkItemListExpression{err: e.err} }
	if e.value == nil { return &WorkItemListExpression{} }
	if !e.value.IsLoaded("workItemList") { return &WorkItemListExpression{err: e.fieldError("workItemList"), root: e.root, path: "workItemList"} }
