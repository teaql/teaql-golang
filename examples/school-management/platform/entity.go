...
func (e *Platform) AttachEntityRoot(root *core.EntityRoot) {
	...
	e.root = root
		for _, child := range e.schoolTypeList.Items() { child.AttachEntityRoot(root) }
...
func (e *Platform) TeaqlPreflightGraph(context *runtime.UserContext) error {
	...
	if !e.markedAsDelete {
		...
		if operation == core.MutationUpdate {
			...
			if !e.IsLoaded("base_url") {
				result := runtime.CheckResult{RuleID: "invalid_type", CanonicalLocation: runtime.Location().Property("base_url"), Message: "Mutation requires a fully loaded entity"}
				return &runtime.RuntimeError{Type: "Check", CheckResults: []runtime.CheckResult{result}}
			}
		...
		checkErr := context.CheckAndFix(&runtime.CheckAndFixInput{Entity: "Platform", Operation: operation, Values: checkedValues})
	...
	for index, child := range e.schoolTypeList.Items() {
		...
		if err := child.TeaqlPreflightGraph(context); err != nil {
			...
			return fmt.Errorf("preflight child from schoolTypeList: %w", err)
		}
	}
