...
func TestTOPN_012CanonicalRelationIndexUsesForeignKeyAndStableID(t *testing.T) {
	ent := core.NewEntityDescriptor("OrderLine").TableName("orderline").
	...
	sqls, err := (&DefaultSqlDialect{Dialect: &TestDialect{}}).SchemaIndexesSqls(ent)
	assert.NoError(t, err)
	assert.Contains(t, sqls, `CREATE INDEX IF NOT EXISTS "IDX_ORDERLINE_ORDER_ID_ID" ON "orderline" ("order_id", "id" DESC)`)
}
